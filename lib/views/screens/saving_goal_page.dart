...
class SavingGoalPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    SavingsController save = Provider.of<SavingsController>(
      context,
      listen: false,
    );
    return Scaffold(
      appBar: AppBar(
        title: Text(save.savingsGoal[argument].name ?? 'BudgetBuddy'),
        centerTitle: true,
      ),
      body: Padding(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  image: DecorationImage(
                    image: MemoryImage(
                      save.savingsGoal[argument].image ??
                          Provider.of<SavingsController>(context).imageBytes!,
                    ),
                    fit:
                        BoxFit
                            .fill, // You can specify how the image should fit within the container
                  ),
                  borderRadius: BorderRadius.circular(12),
                ),
                child: Column(
                ...
                builder: (context, sav, child) {
                  return ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      return save.savingsGoal[argument].name ==
                              sav.transactionListGoal[index].name
                          ? FadeAnimation(
                            ...
                            20,
                            SaveGoalTile(
                              savingName: sav.transactionListGoal[index].name,
                              savingDate: sav.transactionListGoal[index].date,
                              amount: sav.transactionListGoal[index].amount,
                            ),
                          )
                          : Container();
                    },
                            ...
                            GestureDetector(
                              onTap: () async {
                                if (formKey.currentState!.validate()) {
                                  savingGoal.id =
                                      save.savingsGoal[argument].id ?? 0;
                                  savingGoal.name =
                                      save.savingsGoal[argument].name ?? '';
                                  savingGoal.date = DateFormat.yMMMd().format(
                                    DateTime.now(),
                                  );
                                  savingGoal.amount = double.parse(
                                    goalAmountController.text,
                                  );
                                  int id = await DbHelper.dbHelper.insertgoal(
                                    saving: savingGoal,
                                  );
                                  Provider.of<SavingsController>(
                                    context,
                                    listen: false,
