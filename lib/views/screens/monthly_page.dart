...
class MonthlyPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    TransactionController tran = Provider.of<TransactionController>(
      context,
      listen: false,
    );
    DateTime earliestDate =
        tran.transactionList.isNotEmpty
                ...
                Expanded(
                  child: Consumer<TransactionController>(
                    builder: (context, tra, _) {
                      return tra.transactionList.isEmpty
                          ? Center(
                            child: Text(
                              "No Transactions Yet",
                              style: TextStyle(
                          ...
                          ? ListView.builder(
                            ...
                            itemBuilder: (context, index) {
                              TransactionModal tran =
                                  tra.transactionList[index];

                              DateTime dateTime = DateFormat(
                                "MMM d, yyyy",
                              ).parse(tran.date!);

                              return Provider.of<HeatMapController>(
                                        context,
                                      ).selectedDate ==
                                      dateTime
                                  ? FadeAnimation(
